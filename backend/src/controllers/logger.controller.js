...
class LoggerService {
  ...
  initialize(server, app) {
    ...
    this.io.on('connection', socket => {
      ...
      // Adiciona handler para subscrição de inbox
      socket.on('subscribe:inbox', async (data) => {
        const { inboxId } = data
        console.log(`Cliente subscreveu à inbox ${inboxId}`)
        
        // Adiciona socket à sala específica da inbox
        socket.join(`inbox:${inboxId}`)
        
        try {
          // Busca status inicial da inbox
          ...
          console.error('Erro ao buscar status da inbox:', error)
        }
      })
    })
...
// Função para emitir atualizações de status da inbox
export function emitInboxStatus(inboxId, status, qrcode) {
  loggerService.io.to(`inbox:${inboxId}`).emit('inbox:status', {
    inboxId,
    status,
    qrcode
  })
